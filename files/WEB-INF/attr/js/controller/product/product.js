            ...
            var template_choice = ``+
                `<div class="col-lg-3" style="margin-bottom:2%;">` + 
                    `<div class="features-icons-item mx-auto">` + 
                        ...
                        `<a class="btn btn-home btn-sm col-lg-12" onClick="javascript:goToDetailSewa('`+ response.data[i].path +`')" >SEWA</a>` +
                    `</div>` +
                `</div>`;
...
function loadProductDetail(){
    ...
    promise.done(function(response){
        ...
        if(response.data.price_rent_daily=="" && response.data.price_rent_weekly=="" && response.data.price_rent_monthly==""){
...
function goToDetailSewa(nextPath){
    var url = base_url + nextPath;

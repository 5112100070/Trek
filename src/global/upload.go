...
func UploadCompanyImage(companyImage multipart.File, filename string) (string, error) {
	...
	if err != nil {
		Error.Println("func UploadCompanyImage error create new file with name: %v, error: %v", err)
		return result, err
	}
	defer dst.Close()

	_, err = io.Copy(dst, companyImage)
	if err != nil {
		Error.Println("func UploadCompanyImage error write company image to server: ", err)
		return result, err
	}

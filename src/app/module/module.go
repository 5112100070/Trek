...
// GetListModule method to get list module from cgx
...
// GetListFeature method to get list feature from cgx
func (repo moduleRepo) GetListFeature(sessionID string, param ListFeatureParam) (FeatureResponse, error) {
	var result FeatureResponse

	u, _ := url.ParseRequestURI(conf.GConfig.BaseUrlConfig.ProductDNS)
	u.Path = urlConst.URL_ADMIN_GET_LIST_FEATURE
	urlStr := u.String()

	client := &http.Client{}
	req, err := http.NewRequest(http.MethodGet, urlStr, strings.NewReader(""))
	if err != nil {
		log.Println("func GetListFeature error when create new request: ", err)
		return result, err
	}
	...
	resp, errGetResp := client.Do(req)
	if err != nil {
		log.Println("func GetListFeature error when request: ", errGetResp)
		return result, errGetResp
	}
	...
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		log.Println("func GetListFeature error when read request: ", err)
		return result, err
	}
	defer resp.Body.Close()
	...
	errUnMarshal := json.Unmarshal(body, &result)
	if errUnMarshal != nil {
		log.Println("func GetListFeature error when unmarshal: ", errUnMarshal)
		return result, errUnMarshal
	}
